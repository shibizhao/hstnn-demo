...
class HybridLayer2(nn.Module):
    # hybrid RCNN and SCNN block
    def __init__(self, input_channels, rcnn_channels, scnn_channels, conv_kernel_size, pool_kernel_size, conv_stride, pool_stride, fc_padding, fv_padding, avr_pool=True, device="cuda"):
        super(HybridLayer2, self).__init__()
        self.rcnn_channels = rcnn_channels
        self.scnn_channels = scnn_channels
        self.conv_kernel_size = conv_kernel_size
        self.pool_kernel_size = pool_kernel_size
    ...
    def forward(self, inputs, scnn_mem, scnn_spike, rcnn_y):
        ...
        if self.scnn_channels > 0:
            scnn_mem, scnn_spike = self.scnn_mem_update(self.scnn_fc, inputs, scnn_mem, scnn_spike)
            scnn_mem = scnn_mem * self.scnn_mask
            scnn_spike = scnn_spike * self.scnn_mask
            if self.avr_pool == True:
                pooled_scnn_spike = self.scnn_avg_pool(scnn_spike)
            else:
                pooled_scnn_spike = scnn_spike
...
class HNNModel2(nn.Module):
    def __init__(self, nclass, nwin, layer_config, linear_config, buffer_config, dropout=None, device="cuda", union=False):
        ...
        self.hcnn_layers = nn.ModuleList([HybridLayer2(input_channels   = layer['input_channels'],
...
parser.add_argument('--stage1_epochs', type=int, default=100, help='upper epoch limit')
parser.add_argument('--stage2_epochs', type=int, default=100, help='upper epoch limit')
...
# Default parameters
parser.add_argument('--nwin', type=int, default=8,  help='value of time window')
...
parser.add_argument('--nclass', type=int, default=10, help="CIFAR10 class, 10, NOT MODIFY")
...
def data_prepare(arg_args):
    ...
    data_path = arg_args.data + "/"

    train_dataset = MyDataset(data_path + "train.mat", "r", args.nwin)
    test_dataset  = MyDataset(data_path + "test.mat", "r", args.nwin)
        ...
        if batch_idx % args.log_interval == 0 and batch_idx > 0:
            ...
            logger.info('| epoch {:3d} | {:5d}/{:5d} batches | {:5.2f} ms/batch '
                        '| loss {:.5f} | acc {:.3f}%'.format(
                            epoch, batch_idx, int(9000 / args.batch_size), elapsed * 1000 / args.log_interval, cur_loss, 100.0 * correct / total)
                        )
            train_loss = 0
            total      = 0
            correct    = 0
...
def initialize_config():
    ...
    if args.model == "rnn":
        ...
        linear_config[0]["input_channels"] = (layer_config[-1]['rcnn_channels'] + layer_config[-1]['scnn_channels']) * 5 * 5
    elif args.model == "ffs":
        for i in range(len(layer_config)):
            layer_config[i]['rcnn_channels'] = layer_config[i]['cnn_channels'] * (1 - args.ratio)
            layer_config[i]['scnn_channels'] = layer_config[i]['cnn_channels'] * args.ratio
        for i in range(len(buffer_config)):
            buffer_config[i]['rcnn_channels'] = layer_config[i]['rcnn_channels']
            buffer_config[i]['scnn_channels'] = layer_config[i]['scnn_channels']
...
def load_and_shrink(arg_model_name, arg_eff_dict):
    ...
    model_g = HNNModel2(nclass=args.nclass, nwin=args.nwin, layer_config=golden["layer_config"], linear_config=golden["linear_config"], buffer_config=golden["buffer_config"], dropout=args.dropout, device="cuda")

    model_g.load_state_dict(golden['net'])

    for layer_idx in range(len(layer_config)):
        rcnn_list = arg_eff_dict[print_hcnn_weight_name(layer_idx, "rcnn", "fc")]
        scnn_list = arg_eff_dict[print_hcnn_weight_name(layer_idx, "scnn", "fc")]
        rcnn_mask = torch.zeros(1, model_g.hcnn_layers[layer_idx].rcnn_channels, 1, 1).cuda()
        scnn_mask = torch.zeros(1, model_g.hcnn_layers[layer_idx].scnn_channels, 1, 1).cuda()
        for rl in rcnn_list:
            rcnn_mask[0][rl][0][0] = 1
        for sl in scnn_list:
            scnn_mask[0][sl][0][0] = 1
        model_g.hcnn_layers[layer_idx].rcnn_mask = rcnn_mask
        model_g.hcnn_layers[layer_idx].scnn_mask = scnn_mask
    ...
    comm_input_mask  = torch.zeros([linear_config[0]["old_input_channels"] // 25, linear_config[0]["input_channels"] // 25])
    ...
    l0_w = ckpts['net']["linear_layers.0.weight"].view(linear_config[0]["output_channels"], linear_config[0]["old_input_channels"] // 25, 5, 5).permute(3, 2, 0, 1)
...
def get_pruned_model(model_name, logger, union=False, hessian_mode="trace"):
    ...
    pruned_model, model_g = load_and_shrink(model_name, eff_dict)
    test_trained_model(pruned_model, logger)
    test_trained_model(model_g, logger)
    return model_g#, #pruned_model

assert args.model in ["rnn", "snn", "ffs", "hybrid"]

dataset_name = "dvs_cifar"
