...
parser = argparse.ArgumentParser(description='HNN Model on CIFAR10 Dataset')
...
# Activate parameters
parser.add_argument('--batch_size', type=int, default=72, metavar='N', help='batch size')
parser.add_argument('--stage1_epochs', type=int, default=3600, help='upper epoch limit')
parser.add_argument('--stage2_epochs', type=int, default=3600, help='upper epoch limit')
parser.add_argument('--stage1_lr', type=float, default=5e-4, help='upper epoch limit')
parser.add_argument('--stage2_lr', type=float, default=5e-4, help='upper epoch limit')

parser.add_argument('--ratio', type=float, default=0.5, help='snn ratio of hybird network')

# Default parameters
parser.add_argument('--nwin', type=int, default=60,  help='value of time window')
parser.add_argument('--dt',   type=int, default=20000,   help='value of dt (us)')
parser.add_argument('--ds',   type=int, default=4, help='value of ds')
parser.add_argument('--iters_train', type=int, default=60, help='sample')
parser.add_argument('--iters_test',  type=int, default=60, help='sample ms')
parser.add_argument('--test_interval', type=int, default=20, help='How epochs to run before testing')

parser.add_argument('--clip', type=float, default=0.25, help='gradient clipping')
parser.add_argument('--dropout', type=float, default=0.2, help='dropout applied to layers (0 = no dropout)')
parser.add_argument('--seed', type=int, default=1111, help='random seed')
parser.add_argument('--cuda', action='store_true', help='use CUDA')
parser.add_argument('--log-interval', type=int, default=30, metavar='N', help='report interval')
parser.add_argument('--nclass', type=int, default=11, help="CIFAR10 class, 10, NOT MODIFY")
parser.add_argument('--gpu', type=str, default='0', help='gpu number')

args = parser.parse_args()
assert args.nclass == 11
...
in_channels = 2
im_width  = 128 // args.ds
im_height = 128 // args.ds



# copied from weihua's code
def generate_test(gen_test, n_test:int, offset=0):
    input_test, labels_test = gen_test.next(offset=offset)
    input_tests = []
    labels1h_tests = []
    n_test = min(n_test,int(np.ceil(input_test.shape[0] / args.batch_size)))
    for i in range(n_test):
        input_tests.append(torch.Tensor(input_test.swapaxes(0,1))[:,i * args.batch_size:(i+1) * args.batch_size].reshape(args.iters_test,-1,in_channels,im_width,im_height))
        labels1h_tests.append(torch.Tensor(labels_test[:,i * args.batch_size: (i+1) * args.batch_size]))
    return n_test, input_tests, labels1h_tests

def data_prepare(arg_args):

    assert arg_args.ds == 4

    assert arg_args.iters_train == 60 and arg_args.iters_test == 60

    gen_train, _ = create_data(        
                batch_size = arg_args.batch_size,
                chunk_size = arg_args.iters_train,
    ...
    _, gen_test  = create_data(
                batch_size = arg_args.batch_size,
                chunk_size = arg_args.iters_test,
                size = [in_channels, im_width, im_height],
                ds = arg_args.ds,
                dt = arg_args.dt)
    n_test, input_tests, labels1h_tests = generate_test(gen_test, n_test=100, offset=0)
    return gen_train, n_test, input_tests, labels1h_tests

gen_train, n_test, input_tests, labels1h_tests = data_prepare(args)
...
def train(arg_model, optimizers, epoch, logger):
    ...
    inputs = torch.Tensor(inputs.swapaxes(0,1)).reshape(args.iters_train, args.batch_size, in_channels, im_width, im_height)
    ...
    both_loss       = criterion(outputs.cpu(), labels)
    both_loss.backward()
    # torch.nn.utils.clip_grad_norm_(arg_model.parameters(), args.clip)

    optimizers.step()
    
    train_loss = train_loss + both_loss.item()
...
def evaluate(arg_model):
    ...
    input_tests_length = len(input_tests)
    with torch.no_grad():
        for idx in range(input_tests_length):
            inputTest         = input_tests[idx].float().to(device)
            inputTest         = inputTest.permute([1,2,3,4,0])
            outputs           = arg_model(inputTest)
            _, predicted      = torch.max(outputs.data, 1)
            _, labelTestTmp   = torch.max(labels1h_tests[idx].data, 2)
            labelTest, _      = torch.max(labelTestTmp.data, 0)
            total             = total + labelTest.size(0)
            correct           = correct + (predicted.cpu() == labelTest).sum()

    # there is no test loss
...
def train_over_epoch(arg_model, epoch_num, optimizer, logger, arg_model_name):
    ...
    for epoch in range(1, epoch_num + 1):
        train_loss = train(arg_model, optimizer, epoch, logger)
        if epoch > 0 and epoch % args.test_interval == 0:
            test_acc = evaluate(arg_model)
            test_acc_record.append(test_acc)
            logger.info('-' * 83)
            logger.info('| end of epoch {:3d} | time: {:5.2f}s | train loss {:.5f} | test acc {:.3f}%'
                        .format(epoch, time.time() - epoch_start_time, train_loss, test_acc))
            logger.info('-' * 83)
...
def test_trained_model(arg_model, logger):
    test_acc = evaluate(arg_model)
    logger.info('=' * 83)
    logger.info('| Performance on Test Set | test acc {:.5f}'.format(test_acc))
    logger.info('=' * 83)
...
def compute_effective_indices(arg_model, logger, hessian_mode="trace", neuron_nums_dict = None):
    ...
    eff_dict = pruner.make_pruned_model(gen_train, criterion, device, args.ratio, args.seed)
