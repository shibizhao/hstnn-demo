...
class HessianPruner:
        ...
        def _compute_hessian_importance(self, dataloader, criterion, device, batch_size, bptt, ntokens, is_loader, n_v=300):
            ...
            # # sort within every layer   
            # for i in tmp_imp_list:
            #     cur_name = i[0]
            #     cur_list = i[1][0]
            #     nums     = len(cur_list)
            #     tp_list = []
            #     for iid in range(0, nums):
            #         tp_list.append((iid, cur_list[iid]))
    
            #     if cur_name.find("snn") >= 0:

            #         eff_idx_list = []
            #         sorted_tp_list = sorted(tp_list, key=lambda x:x[1])
            #         sorted_tp_list = sorted_tp_list[int((1 - self.snn_ratio) * nums):]
            #         for iid in sorted_tp_list:
            #             eff_idx_list.append(iid[0])
            #         tmp_dict[cur_name] = eff_idx_list

            #     elif cur_name.find("rnn") >= 0 or cur_name.find("lstm") >= 0:
                    
            #         eff_idx_list = []
            #         sorted_tp_list = sorted(tp_list, key=lambda x:x[1])
            #         sorted_tp_list = sorted_tp_list[int((self.snn_ratio) * nums):]
            #         for iid in sorted_tp_list:
            #             eff_idx_list.append(iid[0])
            #         tmp_dict[cur_name] = eff_idx_list
                ...
                # if cur_name.find("snn") >= 0:
                #     layer_bool = [0 for ind in range(nums)]
                #     sorted_tp_list = sorted(tp_list, key=lambda x:x[1])
                #     sorted_tp_list = sorted_tp_list[int((1 - self.snn_ratio) * nums):]

                #     for iid in sorted_tp_list:
                #         layer_bool[iid[0]] = 1
                #     tmp_dict[cur_name] = layer_bool
                # elif cur_name.find("rnn") >= 0 or cur_name.find("lstm") >= 0:
                #     layer_bool = [0 for ind in range(nums)]
                #     sorted_tp_list = sorted(tp_list, key=lambda x:x[1])
                #     sorted_tp_list = sorted_tp_list[int((self.snn_ratio) * nums):]
            ...
            return tmp_dict, self.importances
